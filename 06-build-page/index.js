...
fs.readdir(path.join(__dirname, 'components'), { withFileTypes: true }, (err, files) => {
  if (err) {
    ...
    files.forEach((file) => {
      fs.readFile(path.join(__dirname, 'components', file.name), 'utf8', function (error, fileContent) {
        if (error) throw error;
        template = template.replace(`{{${file.name.split('.')[0]}}}`, fileContent)
  ...
  if (err) {
    ...
    files.forEach((file) => {
      ...
      if (file.isFile() && path.extname(file.name) === '.css') {
        // ===========================================================================
        fs.readFile(
          path.join(__dirname, 'styles', file.name), 'utf8',
          (err, data) => {
            if (err) throw err;

            fs.appendFile(path.join(__dirname, 'project-dist', 'style.css'), String(data), (err) => {
              if (err) throw err;
            });
          },
        );
...
fs.readdir(path.join(__dirname, 'assets'), { withFileTypes: true }, (err, filesSssets) => {
  if (err) {
    console.log(err);
  } else {
    filesSssets.forEach((fileAssets) => {
      if (fileAssets.isDirectory) {
        fs.readdir(path.join(__dirname, 'assets', fileAssets.name), (err, files) => {

          files.forEach((file) => {
            let fileName = String(file);
            mkdir(path.join(__dirname, 'project-dist', 'assets', fileAssets.name), { recursive: true });
            // console.log(`${fileAssets.name} ------ ${fileName}`);
            // ===========================================================================
            fs.copyFile(
              path.join(__dirname, 'assets', fileAssets.name, fileName),
              path.join(__dirname, 'project-dist', 'assets', fileAssets.name, fileName),
              (err) => {
                if (err) {
